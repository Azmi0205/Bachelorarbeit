function fig = plotLoggedSignal2x2(logsout, signalNamesGrid, varargin)
%PLOTLOGGEDSIGNAL2X2 Plot logged Simulink signals in a 2x2 tiled layout,
%thesis-style. Fully self-contained (no other files required).
%
%   plotLoggedSignal2x2(out.logsout, {'Sin_wm','Sin_Te','Sin_ia','Sin_ib'})
%   plotLoggedSignal2x2(out.logsout, {{'Sin_wm','Sin_wm_ref'}, 'Sin_Te', ...
%                                      {'Sin_ia','Sin_ib','Sin_ic'}, 'Sin_vdc'})
%
%   signalNamesGrid must have exactly 4 cells, filled row-major into the
%   grid: {topLeft, topRight, bottomLeft, bottomRight}. Each cell can be
%   a single char/string signal name, or a cell array of char/string
%   names to overlay multiple signals in that tile.
%
%   All four tiles share ONE common x-axis range. The two tiles in the
%   same row share ONE common y-axis range (row 1 = tiles 1-2, row 2 =
%   tiles 3-4). If 'XLim'/'YLim' are left empty, the shared ranges are
%   computed automatically as the union of the tiles' auto-scaled data
%   ranges; if given explicitly, they are applied directly.
%
%   Optional name-value pairs:
%     'XLabel'         - 1x4 cell array of LaTeX x-axis labels (per tile).
%                        Use [] or '' entries to leave a tile without an
%                        x-axis label text (default: no labels)
%     'YLabel'         - 1x4 cell array of LaTeX y-axis labels (per tile).
%                        Use [] or '' entries to leave a tile without a
%                        y-axis label text (default: no labels)
%     'Title'          - 1x4 cell array of LaTeX tile titles (per tile).
%                        Use [] or '' entries to leave a tile untitled
%                        (default: no titles)
%     'Legend'         - 1x4 cell array of cell arrays of LaTeX legend
%                        labels (per tile)
%     'XLim'           - single [xmin xmax] applied to all four tiles
%                        (default: [] = auto, computed from the data)
%     'YLim'           - 1x2 cell array {yLimRow1, yLimRow2}, each either
%                        [ymin ymax] or [] for auto (default: {[],[]})
%     'ShowXTickLabels'- 1x4 logical array; false hides only the numeric
%                        tick labels on that tile's x-axis. The axis
%                        label text (set via 'XLabel') is unaffected
%                        (default: all true)
%     'ShowYTickLabels'- 1x4 logical array; false hides only the numeric
...
%                        If empty, no export is performed. (default: '')
    ...
    if numel(signalNamesGrid) ~= nTiles
        error('signalNamesGrid must contain exactly %d cells (2x2, row-major).', nTiles);
    end
    ...
    fig = figure('Color','w','Units','centimeters','Position',[1 -1 60 44]);
    t_layout = tiledlayout(fig, 2, 2, 'TileSpacing','compact','Padding','compact');
    rows = {[1 2], [3 4]};
    ...
    % --- Ensure a single common x limit across all four tiles ---
    ...
    % --- Ensure a single common y limit within each row ---
    for r = 1:numel(rows)
        idx = rows{r};
    ...
    set(fig,'PaperSize',[60 44]);
    set(fig,'PaperPosition',[0 0 60 44]);
    if ~isempty(p.Results.ExportName)
        exportgraphics(fig, [char(p.Results.ExportName) '_2x2.pdf'], 'ContentType','vector');
    end
end
